...
##   sample-annotation validation; retires superseded DE tables
...
required_pkgs <- c(
  ...
  "grid",
  "scales",
  "ashr"     ## shrinkage estimator for lfcShrink() under cell-means contrasts
)
...
tryCatch({
  ...
  validate_sample_annotation <- function(cm, annot) {
    ...
    ## Keep the gene names: rbind() of plain vectors produces a matrix with NO
    ## rownames, so the "Y-gene score used" line printed empty and you could not
    ## tell which genes the check actually relied on.
    yfound <- ygenes[!vapply(lapply(ygenes, getg), is.null, logical(1))]
    ymat <- do.call(rbind, lapply(yfound, getg))
    if (!is.null(ymat) && length(yfound) > 0) rownames(ymat) <- yfound
  ...
  if (GENERATE_PLOTS) {
    ...
    pca_tissue <- prcomp(t(vst_mat[top_var_idx, , drop = FALSE]), scale. = FALSE)
  ...
  for (cn in contrast_names) {
    ...
    ## Shrunken effect sizes (ashr handles arbitrary contrasts under the
    ## ~ 0 + GroupDepot cell-means design). Exported as an extra column and
    ## suitable for ranking/visualisation; DEG calling below is UNCHANGED
    ## (still MLE log2FoldChange + padj), and GSEA still ranks on the Wald
    ## stat, so validated counts and pathway inputs are unaffected.
    res_shrunk <- tryCatch(
      lfcShrink(dds_tissue, contrast = contrast_definitions[[cn]],
                type = "ashr", res = res, quiet = TRUE),
      error = function(e) {
        cat("[WARN] lfcShrink (ashr) failed: ", conditionMessage(e),
    ...
    ## so rownames() here returns "1","2",... and would silently feed
    ## non-gene IDs to ORA/GSEA. gene_name is a real column, so it is safe.
    up_genes <- tt %>%
    ...
    if (GENERATE_PLOTS) {
      ...
      if (length(goi_in_data) >= 2) {
        ...
        if (!is.na(depot_h)) {
          ...
          ## Base indexing (dplyr::filter drops the sample row names we need here)
          heat_ctrls <- rownames(heat_meta)[heat_meta$Genotype == "CTL"]
