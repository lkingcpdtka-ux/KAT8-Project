...
## =========================================================
## KAT8 bulk RNA-seq - PART 3: FGSEA PATHWAY ANALYSIS (CELLS)
## =========================================================
## This script performs Gene Set Enrichment Analysis (fgsea)
## for GO:BP, KEGG, WikiPathways, and Hallmark pathways
## using ranked gene lists from Part 1
##
## FEATURES:
## - Use DESeq2 Wald statistic for ranking
## - Direction-specific pathway plots
## - Leading edge gene exports
...
tryCatch({
  ...
  run_fgsea_analysis <- function(ranked_genes, contrast_name,
    ...
    if (length(results) > 0) {
      for (db_name in names(results)) {
        ...
        ## Skip simplified version for plotting
        ...
        if (nrow(plot_data) > 0) {
          ## Bar plot: Use consistent colors: orange for up, blue for down
          ...
          ## Dotplot - shows NES and significance
  ...
  cat("\n=== RUNNING fgsea FOR CELLS CONTRAST ===\n")
  ...
  sanity_file <- paste0("fgsea_sanity_check_cells_", run_tag, ".csv")
  ...
  cat("\n==========================================================\n")
  cat("=== SANITY CHECK SUMMARY: fgsea PATHWAY ANALYSIS (CELLS) ===\n")
  cat("==========================================================\n")
  ...
  session_file <- file.path(outdir, "logs", paste0("sessionInfo_fgsea_cells_", run_tag, ".txt"))
  sink(session_file)
  ...
  err_file <- paste0("ERROR_fgsea_cells_", run_tag, ".txt")
  writeLines(
    c(
      "fgsea script error:",
      conditionMessage(e),
