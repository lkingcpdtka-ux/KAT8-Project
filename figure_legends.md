...
Volcano plot of differential gene expression in gWAT comparing KAT8 KD to CTL. Each point represents a gene tested by DESeq2. The x-axis shows the log2 fold change (KD/CTL) and the y-axis shows --log10(FDR-adjusted p-value). Genes are colored by significance category: red indicates significantly upregulated genes (FDR < 0.05 and log2FC >= 1.0), blue indicates significantly downregulated genes (FDR < 0.05 and log2FC <= --1.0), and grey indicates non-significant genes. Dashed horizontal and vertical lines mark the FDR and fold-change thresholds, respectively. Gene labels were selected using an ORA-informed strategy: for each direction (up/down), differentially expressed genes were intersected with significant KEGG ORA pathways (FDR < 0.05) and each gene was assigned a pathway membership count (number of enriched KEGG pathways containing that gene). Predicted and unannotated genes (Gm*, Rik) were excluded. Genes with at least one pathway membership were ranked by a composite score combining z-scored pathway count, z-scored absolute log2 fold change, and z-scored --log10(FDR), and the top 20 genes per direction are labeled. When fewer than 3 ORA-supported genes were available for a direction, ranking fell back to DE significance alone (adjusted p-value, then absolute fold change). *Kat8* is highlighted as a mandatory biological anchor with a yellow-filled point and bold label if it passes DE thresholds. Gene labels are positioned using repulsive text placement to minimize overlap.

### Figure: Volcano Plot -- iWAT KD vs CTL (Volcano_iWAT_KD_vs_CTL)

Volcano plot of differential gene expression in iWAT comparing KAT8 KD to CTL. All plot conventions, color scheme (red = upregulated, blue = downregulated, grey = non-significant), significance thresholds (FDR < 0.05, |log2FC| >= 1.0), and ORA-informed gene labeling strategy are identical to the gWAT volcano plot described above. Up to 20 genes per direction are labeled based on the composite score of KEGG pathway membership, fold change magnitude, and statistical significance. *Kat8* is highlighted as a mandatory biological anchor.
